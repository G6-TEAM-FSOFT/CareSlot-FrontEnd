...
import { History, RefreshCw, Pill, X } from 'lucide-react';
      ...
      <div className="bg-white rounded-3xl max-w-4xl w-full border border-slate-200 shadow-2xl overflow-hidden animate-fadeIn space-y-0">
        ...
        <div className="bg-gradient-to-r from-indigo-700 to-purple-800 text-white p-6 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-indigo-100 font-mono">Bệnh nhân: {visit?.patientName || selectedEncounter?.patientName || 'Nguyễn Minh Anh'}</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="p-6 space-y-4 max-h-[75vh] overflow-y-auto text-xs">
          {loadingHistory ? (
                ...
                <div key={histVisit.id || index} className="p-5 bg-slate-50 rounded-2xl border border-slate-200 space-y-3 hover:border-indigo-300 transition shadow-sm">
                  ...
                  <div className="grid sm:grid-cols-2 gap-3">
                    <div>
                      ...
                      <strong className="text-slate-800">{histVisit.primaryDoctorName || 'BS. Trần Hoàng Minh'}</strong>
                    </div>
                    <div>
                      <span className="text-[10px] text-slate-500 font-bold uppercase block">Chẩn Đoán Đã Ghi Nhận:</span>
                      <strong className="text-emerald-800">{histVisit.prescription?.diagnosisNote || histVisit.diagnosis || 'Viêm dạ dày - tá tràng cấp tính (K29.1)'}</strong>
                    </div>
                  </div>
            ...
            /* Fallback sample history when no records exist yet in database */
            <div className="space-y-4">
              <p className="font-medium">Bệnh nhân chưa có lịch sử khám</p>
            </div>
          )}
        </div>
